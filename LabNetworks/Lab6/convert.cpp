...
public:
  ...
  void processPacketData(char event, float timestamp, int toNode,
    ...
    if (packet_sent != 0)
      f << timestamp << " " << (float)(packet_drop) / packet_sent << '\n';
  }
};
...
public:
  ...
  void processPacketData(char event, float timestamp, int toNode,
    ...
    if (packet_sent != 0)
      f << timestamp << " " << (float)(packet_recv) / packet_sent << '\n';
  }
};
...
public:
  ...
  void processPacketData(char event, float timestamp, string packetType) {
    if (event == '+') {
      total_packets++;
      if (packetType == "rtProtoDV" || packetType == "rtProtoLS") {
        control_packets++;
      }
    }
    f << timestamp << " " << (float)(control_packets) / (total_packets) << '\n';
  }
};
